...
various optimizations in minimax algorithm and in StaticEvaluation
track elapsed time to find ai_move through minimax algorithm
*/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
#define MAX_DEPTH 5 // max depth reached by minimax function
...
{	
	...
	int j;
	int max_depth, score, possible_moves[SPOTS];
	...
	for (i=0; i<SPOTS; i++)
		if (insert (+1, i))
		{
			score = minimax (count, 0, max_depth, alpha, beta, 0);
...
int minimax (int move_counter, int depth, int max_depth, int alpha, int beta, int Maximising_Turn)
{	
	int i;
	...
	if (Maximising_Turn)
	{
		...
		for (i=0; i<SPOTS; i++)
			if (insert (+1, i))
			{
				score = minimax (move_counter, depth+1, max_depth, alpha, beta, 0);
				tab[i]=0; // reset the spot occupied
